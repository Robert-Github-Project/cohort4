...
// Create function to add strings with the refrain
const refrainString = (refrain) => {
    let string = "";
    string += refrain.replace(/ {2}/g, "<br>");
    string += "<br/><br/>";
    return string;
  };
  
const songCreator = lyrics => {
    //Using promise to handle potential asynchronous actions
    return new Promise((resolve, reject) => {

            let error = false;
            //adding <breaks> to create proper lines as instructed
            let song = lyrics.intro.replace(/ {2}/g, "<br>");
            song += "<br/><br/>";
            //Using looping to add all choruses to our output.
            for (let i = 0; i < lyrics.choruses.length; i++) {
                song += lyrics.choruses[i].chorus.replace(/ {6}/g, "<br/>");
                song += "<br/><br/>";
                //Ading refrain
                song += refrainString(lyrics.refrain);
                song += refrainString(lyrics.refrain);
        }
        //Add Ending
        song += lyrics.ending;
        if (!error) {
            ...
            reject("Error: Unable to create song");
        }
    });
};
...
songCreator(lyrics)
//For fun I created an index.html file with <p> with an id of "output" to send the song to.
//I realize using innerHTML is not ideal but I figured it would be fine for this assignment :)
